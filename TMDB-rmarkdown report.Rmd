...
1) To develop a machine learning model to predict the internaltion box office revenue and submit to Kaggle TMDB Box Office Competition. My target is reducing RMSLE as much as possible.

2) To practice data exploratory analysis, text mining, data visualization using tidyverse, ggplot2, caret, lubridate.

3) To improve my business writing and reporting skills, using `rmarkdown`. 
...
+ Step 3: **modeling and performance evaluation**. we develop predicting model using Caret package and evaluate its effectiveness by using RMLSE. The RMLSE are following:
...
+ Step 4: **final evaluate the model** on the test set and submit to Kaggle's leaderboards.
...
url_additional <- url("https://raw.githubusercontent.com/billynguyenlss/TMDB-Box/master/data/personal%20additional%20data/full_additional_features.csv")
addition_data <- read.csv(url_additional, na.strings=c("", '#N/A', '[]', '0'))
```

```{r}
# extract the true_rating value from additional data
test_y <-left_join(test_set, addition_data, by = "imdb_id") %>%
  select(revenue)
test_y$revenue[is.na(test_y$revenue)] <- median(test_y$revenue, na.rm = T)
test_y <- test_y$revenue
```
...
Adding `country` to the linear model and see
...
```{r}
df <- df %>% left_join(addition_data, by = "imdb_id")
names(df)
```
...
```{r}
dat <- df %>% 
  select(budget, popularity.y, runtime.x,
         avg_revenue_per_collection,
         movies_per_company, ROI_per_company,
         movies_per_director, avg_revenue_per_director,
         n_cast, n_crew, n_spoken_languages,
         revenue.x, vote_average, vote_count)

dat <- df %>% select(imdb_id, budget, popularity.x, runtime.x, number_genres,
                     number_of_company, n_spoken_languages, 
                     n_cast, n_crew, 
                     avg_revenue_per_collection, 
                     movies_per_director, avg_revenue_per_director,
                     avg_revenue_per_cast.x, avg_revenue_per_cast.y,
                     ROI_per_company, movies_per_company,
                     vote_average, vote_count)

df$budget[df$imdb_id == "tt0475783"] <- 8000000
df$budget[df$imdb_id == "tt1408101"] <- 190000000
df$revenue[df$imdb_id == "tt1408101"] <- 228778661
df$budget[df$imdb_id == "tt1913166"] <- 10000000
...
dat$new_budget.x <- log10(dat$new_budget.x)
dat$avg_revenue_per_collection <- log10(dat$avg_revenue_per_collection)
dat$avg_revenue_per_director <- log10(dat$avg_revenue_per_director)
```
...
yhat_one <- predict(fit_one, newdata = dat[3001:7398,-12])
mean(is.na(yhat_one))
```

```{r}
RMLSE(10^yhat_one, test_y)
```
...
```{r}
sample_submission$revenue <- 10^yhat_one
names(sample_submission) <- c("id","revenue")
write.csv(sample_submission, file = "submission_one.csv", row.names = F)
```
