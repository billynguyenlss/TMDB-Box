...
```{r}
# extract the true_rating value from additional data for training set
train_y <- train_set %>% left_join(additional_data, by = "imdb_id") %>%
  mutate(new_revenue = ifelse(is.na(new_revenue), median(new_revenue, na.rm = T), new_revenue)) %>%
  pull(new_revenue)
```
...
df <- df %>% 
  ...
  mutate(release_date = new_release_date)
```
...
df$budget <- df$new_budget
df$new_revenue[is.na(df$new_revenue)] <- df$revenue
