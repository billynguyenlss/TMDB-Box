...
First of all, we could see the minimum revenue was 1 and the maximum revenue was `r max(train_set$revenue)`. In a little more detail,, there were total
...
train_set %>% dplyr::select(budget, revenue) %>%
  ...
  formattable(`count` = color_bar("#FA614B"))
```
...
```{r, fig.width=8,fig.height=3,echo=FALSE,message=FALSE}
ggplotly(
...
```{r, fig.width=7, fig.height=2.5, echo=FALSE,message=FALSE}
...
subplot(
  ...
  ggplotly(df %>% ggplot(aes(release_month)) +
...
subplot(
    ggplotly(df %>% group_by(weekday) %>% summarize(count = n()) %>%
...
```{r, fig.width=8, fig.height=3.5,echo=FALSE,message=FALSE}
...
```{r,fig.height=3, fig.width=8,echo=FALSE,message=FALSE}
# figure out the number of company per movies and revenue change
subplot(
  ggplotly(df %>% mutate(number_of_company = factor(number_of_company)) %>%
...
first_company_summary %>% arrange(desc(movies_per_company)) %>%
  ...
  formattable(list(`movies_per_company` = color_bar("#FA614B"),
                   ...
                   `ROI_per_company` = color_bar("#CCCCFF")))
```
...
`r RMSLE(ensembles$rf, test_y)`. Therefore I used random forest for remain experiments. 
...
```{r}
ggplotly(final_results %>% gather(3:7, key = "method", value = "RMSLE") %>%
...
`r round(results_exp4$rf,3)`. It might cause of the unbalancing data between train and test data. Certainly, the train data have 3000 rows (approx ~40% total observations) and test data have 4398 observations (approx ~60% total observations). To be more precise, approx 65% movie's collection appeared in both train and test data. This issue might lead to an inaccuracy predicted results for a movie with unseen predictors as well.
...
`r round(results_exp2$rf,3)`)
 ...
 `r round(results_exp2$glmboost,3)`), Bayesian Generalized Linear Model (bayesglm) (RMSLE 
 `r round(results_exp2$bayesglm,3)`), Linear Model (lm) (RMSLE
`r round(results_exp2$lm,3)`).
Three approach for feature selection were experimented with no significant difference on modeling performance (Univariate Filter RMSLE `r round(confounding_features_results$Univariate_Filters,3)`
, revenue-group features RMSLE 
`r round(confounding_features_results$Revenue_group_Features,3)`
, budget-group features RMSLE 
`r round(confounding_features_results$Budget_group_Features,3)`).
...
(RMSLE `r round(RMSE(yhat_exp4, test_set$revenue),3)`) 
...
`r round(final_model2_RMSLE,3)`) belong to model using revenue-group features. 
